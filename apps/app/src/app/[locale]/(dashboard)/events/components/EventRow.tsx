...
export function EventRow({ event, locale }: EventRowProps) {
  ...
  return (
    <tr 
      ...
      className="border-t border-[#1F1F27] hover:bg-[#1A1A21] hover:border-[#1F1F27] transition-colors duration-50 cursor-pointer group relative"
      tabIndex={0}
      role="link"
    ...
    >
      ...
      <td className="px-4 py-3" style={{ color: colors.text.secondary }}>
        ...
        {showDaysIndicator && (
          <span 
            ...
            style={{ 
              backgroundColor: daysUntil <= 7 
                ? colors.status.pending.bg 
                : colors.status.draft.bg, 
              color: daysUntil <= 7 
                ? colors.status.pending.text 
                : colors.status.draft.text 
            }}
          >
            {daysUntil}d
          </span>
      ...
      <td className="px-4 py-3">
        ...
        <div className="flex flex-col">
          <div style={{ color: colors.text.secondary }}>{event.location}</div>
          {event.attendeeCount && event.attendeeCount > 0 && (
            <div className="flex items-center text-xs mt-0.5" style={{ color: colors.text.tertiary }}>
      ...
      <td className="opacity-0 group-hover:opacity-100 pr-2 transition-opacity duration-50">
        <div className="flex space-x-1">
          <button 
            className="p-1 rounded hover:bg-[#252530] transition-colors duration-50"
